...
export function Cottage({
  ...
  const handlePointerMove = (event) => {
    ...
    if (isRotating) {
      ...
      cottageRef.current.rotation.y += delta * 0.01 * Math.PI;
  ...
  const handleKeyDown = (event) => {
    if (event.key === "ArrowLeft") {
      ...
      cottageRef.current.rotation.y += 0.005 * Math.PI;
  ...
  useFrame(() => {
    ...
    if (!isRotating) {
      ...
      cottageRef.current.rotation.y += rotationSpeed.current;
    } else {
      // When rotating, determine the current stage based on island's orientation
      const rotation = cottageRef.current.rotation.y;
    ...
    return (
      <a.group ref={cottageRef} {...props} dispose={null}>
