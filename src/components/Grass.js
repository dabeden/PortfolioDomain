...
export default function Grass({ options = { bW: 0.12, bH: 1, joints: 5 }, width = 100, instances = 50000, ...props }) {
  ...
  const groundGeo = useMemo(() => {
    ...
    // Modify the vertex positions to create terrain
    for (let i = 0; i < position.count; i++) {
      const x = position.getX(i);
      const z = position.getZ(i);
      const y = getYPosition(x, z); // Apply your noise function
      position.setY(i, y);
    }
    
    // Update normals to reflect the changes in position
...
function getYPosition(x, z) {
  // Use the simplex function directly
